  ...
  return (
    <div className=" pb-3 z-50 bg-white">
      ...
      <div  className="pt-3  flex flex-col gap-[50px] justify-center"> 
          ...
          <div className={classNames(tab === 'lecture' ? 'text-green-500' : 'text-black', 'text-xl border border-black px-4 py-2 rounded-md cursor-pointer hover:text-green-500 hover:border-green-500')} onClick={() =>{ router.push('/admin/review'); setTab('review')} }>성공사례 수정 페이지</div>
          ...
          <div className={classNames(tab === 'review' ? 'text-green-500' : 'text-black', 'text-xl border border-black px-4 py-2 rounded-md cursor-pointer hover:text-green-500 hover:border-green-500')}  onClick={() => {router.push('/admin/reviewAdmin'); setTab('review')  }}>성공 사례 및 공지사항 발행</div>
